...
export class ImageCompress {
  ...
  /**
   * return a promise with the new image array data and image orientation array
   */
  static uploadMultipleFiles(render: Renderer2): Promise<{ images: string[], orientations: DOC_ORIENTATION[] }> {

    const promise: Promise<{ images: string[], orientations: DOC_ORIENTATION[] }> = new Promise(function (resolve, reject) {
      ...
      render.listen(inputElement, 'change', ($event) => {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          myReader.onloadend = (e) => {
            try {
              ImageCompress.getOrientation(files[i], orientation => {
                imageArray.images.push(myReader.result)
                imageArray.orientations.push(orientation);
                if (files.length == imageArray.images.length) {
                  resolve(imageArray);
                }
              });
            } catch (e) {
